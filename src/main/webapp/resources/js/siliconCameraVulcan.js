...
function connect() {
    ...
    stompClient.connect({}, function(frame) {
        ...
        stompClient.subscribe('/topic/device-model-silicon-camera-vulcan-trm251', function(resultTrm){
        ...
        stompClient.subscribe('/topic/generate-chart-silicon-camera-vulcan-raspberry', function(result){
...
let vTitle = 'Обєкт/Київгума/силіконова зона/камера вулканізації'+' з '+vStart.toString()+' по '+vEnd.toString();
...
function generateNewChartTitle(start, end) {
    config.options.title.text = 'Обєкт/Київгума/силіконова зона/камера вулканізації'+' з '+start.toString()+' по '+end.toString();
}
...
function sendChartBody() {
    ...
    stompClient.send("/app/generate-chart-silicon-camera-vulcan-raspberry", {}, dataChart);
}
...
function drawInRealTime(parsed) {
    ...
    let y1 = parsed.inputRegister0;
    let y2 = parsed.inputRegister1;
