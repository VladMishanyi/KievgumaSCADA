...
function connect() {
    ...
    stompClient.connect({}, function(frame) {
        ...
        stompClient.subscribe('/topic/generateChartKotelnyaParMikrolITM4', function(result){
...
var vTitle = 'Объект/Киевгума/Котельня/Мониторинг пара'+' с '+vStart.toString()+' по '+vEnd.toString();
...
function generateNewChartTitle(start, end) {
    config.options.title.text = 'Объект/Киевгума/Котельня/Мониторинг пара'+' с '+start.toString()+' по '+end.toString();
}
...
function sendChartBody() {
    ...
    stompClient.send("/app/generateChartKotelnyaParMikrolITM4", {}, dataChart);
}
...
function showBody(body){
    ...
    $("#kotelnyaParMikrolITM4VirabotkaPara").text(channel1);
    $("#kotelnyaParMikrolITM4DavleniePara").text(channel2);
    $("#kotelnyaParMikrolITM4TemperaturaPara").text(channel3);
    $("#kotelnyaParMikrolITM4TemperaturaKotelny").text(channel4);
}
...
function buildChart(x, y1, y2, y3, y4) {
...
function addLastElementToChart(X1, Y1, Y2, Y3, Y4) {
    globalX.push(X1);
    globalY1.push(Y1);
    globalY2.push(Y2);
    globalY3.push(Y3);
...
function drawInRealTime(parsed) {
    ...
    if (config.data.labels.length < buffer){
        addLastElementToChart(x, y1, y2, y3, y4);
    }
    if (config.data.labels.length >= buffer){
