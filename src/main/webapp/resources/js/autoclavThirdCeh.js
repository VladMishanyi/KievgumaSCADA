...
function connect() {
    ...
    stompClient.connect({}, function(frame) {
        ...
        stompClient.subscribe('/topic/chart-body', function(result){
...
function buildChart(x, y1, y2) {
    ...
    config.data.datasets.data[0] = y1;
    config.data.datasets.data[1] = y2;
...
function addLastElementToChart(X1, Y1, Y2) {
    ...
    config.data.datasets.data[0].push = Y1;
    config.data.datasets.data[1].push = Y2;
    window.myLine.update();
}
...
function drawInRealTime(parsed) {
    ...
    var inSet = parsed.value1;
    if (config.data.labels.length < buffer){
        addLastElementToChart(inSet, inSet);
    }
    if (config.data.labels.length >= buffer){
